        ...
        <div key={comment.id} className={`${parentId ? 'ml-6 md:ml-12 mt-4 relative border-l-2 border-slate-100 pl-4' : 'mt-10'}`}>
          <div className="flex gap-3 md:gap-4 items-start group">
            <img 
              ...
              className="w-8 h-8 md:w-10 md:h-10 rounded-lg border border-slate-100 shadow-sm object-cover bg-white" 
              alt="" 
            />
            <div className="flex-1 space-y-1">
              <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm group-hover:shadow-md transition-all">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 border-b border-slate-50 pb-6">
        ...
        {user && (
          <div className="flex items-center gap-3 bg-slate-50 p-1.5 pr-4 rounded-xl border border-slate-100">
             <img src={user.user_metadata?.avatar_url} className="w-8 h-8 rounded-lg object-cover border-2 border-white shadow-sm" alt="" />
      ...
      <div className="mb-12">
        {user ? (
          <form onSubmit={handleSubmit} className="bg-slate-50 p-6 rounded-xl border border-slate-100 shadow-inner">
            {replyTo && (
              <div className="mb-4 flex items-center justify-between bg-[#002040] p-3 px-4 rounded-lg text-white shadow-md">
                <div className="flex items-center gap-2">
            ...
            <div className="flex items-center justify-between pt-4 border-t border-slate-200 mt-4">
              ...
              <button 
                ...
                type="submit" 
                className="bg-[#002040] text-white px-10 py-2.5 rounded-lg font-black text-[10px] uppercase tracking-widest hover:bg-blue-900 transition-all active:scale-95 disabled:opacity-50"
              >
          ...
          <div className="bg-slate-50 p-12 rounded-2xl text-center border border-dashed border-slate-200">
            ...
            <button onClick={handleLogin} className="inline-flex items-center gap-4 bg-white border-2 border-slate-200 px-10 py-4 rounded-xl font-black text-[11px] uppercase tracking-widest hover:border-[#002040] hover:text-[#002040] transition-all shadow-sm">
