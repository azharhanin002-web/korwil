...
// Paksa data selalu fresh agar angka views akurat
...
const ptComponents = {
    ...
    image: ({ value }: any) => {
      ...
      return (
        <div className="my-10 overflow-hidden rounded-xl border-4 border-white shadow-xl ring-1 ring-blue-50 bg-slate-50">
          <Image src={urlFor(value).url()} alt={value.alt || "Gambar Artikel"} width={800} height={500} className="w-full object-cover" />
          {value.caption && <p className="bg-blue-50 py-3 text-center text-[10px] font-black uppercase tracking-widest text-blue-600">{value.caption}</p>}
        </div>
      );
    },
  },
  block: {
    ...
    blockquote: ({ children }: any) => (
      <blockquote className="border-l-4 border-blue-600 pl-6 italic my-10 text-blue-900 bg-blue-50/50 py-8 rounded-r-xl shadow-inner font-medium text-xl">
        "{children}"
      </blockquote>
    ),
  },
...
export default async function ArtikelDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Ambil data (useCdn: false agar views langsung update di layar)
  ...
  return (
    <main className="min-h-screen bg-white pb-24 font-sans">
      <div className="max-w-7xl mx-auto px-4 md:px-6 pt-12 md:pt-20">
        ...
        {/* --- BREADCRUMB (NAVIGASI PROFESIONAL) --- */}
        ...
        <header className="mb-12 max-w-4xl mx-auto md:mx-0">
          <div className="flex justify-start mb-6">
            <div className="inline-flex items-center gap-2 bg-blue-100 text-blue-700 text-[10px] font-black px-4 py-1.5 rounded-lg uppercase tracking-[0.2em] shadow-lg shadow-blue-200/50 border border-blue-200">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-8 w-full">
            ...
            <div className="mt-16 pt-10 border-t border-slate-100 space-y-12">
               <div className="block w-full">
                  <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-6 italic">Bagikan Pengetahuan Ini:</h4>
          ...
          <aside className="lg:col-span-4 w-full h-fit sticky top-28">
            <div className="space-y-8">
              ...
              {toc.length > 0 && (
                <div className="bg-slate-50 p-7 rounded-xl border border-slate-100 shadow-sm">
                  <div className="flex items-center gap-3 mb-6 border-b border-blue-100 pb-4">
                    ...
                    <h4 className="font-black text-blue-900 uppercase tracking-tighter text-sm italic">Isi Artikel</h4>
                  </div>
                    ...
                    <Link key={school._id} href={`/sekolah/${school.slug}`} className="flex items-center justify-between group">
                      <div className="flex items-center gap-4">
                        <div className="relative">
                          <div className="w-10 h-10 rounded-xl bg-white flex-shrink-0 flex items-center justify-center overflow-hidden border border-blue-100 shadow-sm group-hover:border-blue-500 transition-all transform group-hover:rotate-3">
                            {school.logo ? <img src={urlFor(school.logo).url()} className="w-full h-full object-contain p-1.5" alt={school.name} /> : <User size={16} className="text-blue-200"/>}
                          </div>
                          <div className={`absolute -top-2 -right-2 w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black text-white shadow-lg border-2 border-white ${idx === 0 ? 'bg-yellow-500' : 'bg-blue-600'}`}>{idx + 1}</div>
                        </div>
                        <span className="text-[11px] font-black uppercase tracking-tight text-slate-800 group-hover:text-blue-600 line-clamp-1">{school.name}</span>
                      </div>
                      <div className="bg-white px-3 py-2 rounded-xl border border-blue-100 text-center min-w-[60px] shadow-sm group-hover:bg-blue-600 group-hover:text-white transition-all">
              ...
              {/* WIDGET 3: POPULER */}
              <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-md">
        ...
        <div className="mt-32 border-t border-slate-100 pt-16">
          <div className="flex items-center gap-3 mb-10">
            ...
            <h3 className="text-2xl font-black text-slate-800 uppercase tracking-tighter italic text-shadow">Karya Guru Lainnya</h3>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {post.related?.map((rel: any) => (
              <Link href={`/artikel/${rel.slug}`} key={rel._id} className="group flex flex-col bg-white rounded-xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-500">
                ...
                <div className="p-5 flex flex-col flex-1">
                  <div className="text-[9px] font-black text-blue-600 uppercase mb-2 tracking-widest">{rel.category || "Inovasi"}</div>
        ...
        <div className="mt-24 flex justify-center pb-10">
          <Link href="/artikel" className="group flex items-center gap-6 text-[11px] font-black text-slate-400 uppercase tracking-[0.4em] hover:text-blue-600 transition-all">
            <div className="bg-slate-50 group-hover:bg-blue-600 group-hover:text-white p-5 rounded-full shadow-sm border border-slate-100 group-hover:shadow-blue-200 group-hover:shadow-lg transition-all">
              <ArrowLeft size={20} />
            </div>
            Daftar Artikel Guru
          </Link>
