...
const ptComponents = {
    ...
    image: ({ value }: any) => {
      ...
      return (
        <div className="my-10 overflow-hidden rounded-xl border-4 border-white shadow-xl ring-1 ring-blue-50 bg-slate-50">
          ...
          />
          {value.caption && <p className="bg-blue-50 py-3 text-center text-[10px] font-black uppercase tracking-widest text-blue-600 border-t">{value.caption}</p>}
        </div>
      );
    },
  },
  block: {
    h2: ({ children }: any) => {
      ...
      return <h2 id={id} className="text-2xl font-black mt-14 mb-6 text-blue-900 uppercase border-l-4 border-blue-600 pl-4 tracking-tight scroll-mt-28">{children}</h2>;
    },
    h3: ({ children }: any) => {
      const id = children[0].toString().toLowerCase().replace(/\s+/g, "-");
      return <h3 id={id} className="text-xl font-black mt-10 mb-4 text-slate-800 uppercase tracking-tight scroll-mt-28">{children}</h3>;
    },
    // --- SMART AUTO-QUOTE & JEDA MB-7 (PAS) ---
    normal: ({ children }: any) => {
    ...
    // FITUR QUOTE BLOCK (KHUSUS)
    blockquote: ({ children }: any) => (
      <div className="relative my-12 group">
        <div className="absolute -left-2 top-0 bottom-0 w-1.5 bg-blue-600 rounded-full"></div>
        <blockquote className="pl-10 pr-6 py-10 bg-blue-50/40 rounded-r-2xl shadow-inner relative overflow-hidden">
...
export default async function ArtikelDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const currentUrl = `https://www.korwilbarat.web.id/artikel/${slug}`;
  const isVideo = post.category === "Video";
  ...
  return (
    <main className="min-h-screen bg-white pb-24 font-sans pt-12 md:pt-20">
      <div className="max-w-7xl mx-auto px-4 md:px-6" suppressHydrationWarning>
        ...
        {/* --- 1. NAVIGASI (BREADCRUMB) - ANAK PERTAMA BIAR GAK ERROR --- */}
          ...
          <div className="lg:col-span-8 w-full">
            ...
            <div className="mt-20 pt-10 border-t border-slate-100 space-y-16">
               <div className="block w-full">
                  <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-8 italic">Bagikan Pengetahuan Ini:</h4>
          ...
          <aside className="lg:col-span-4 w-full h-fit sticky top-28 space-y-12">
            {toc.length > 0 && (
              <div className="bg-slate-50 p-8 rounded-xl border border-slate-100 shadow-sm">
                <div className="flex items-center gap-3 mb-8 border-b border-blue-100 pb-4">
                  <List size={20} className="text-blue-600" /><h4 className="font-black text-blue-900 uppercase tracking-tighter text-sm italic">Struktur Artikel</h4>
                </div>
            ...
            <div className="bg-white rounded-xl shadow-2xl border border-slate-100 overflow-hidden relative">
              <div className="bg-gradient-to-r from-blue-700 to-indigo-800 p-6 flex justify-between items-center text-white">
                <div className="flex items-center gap-3"><TrendingUp size={22} className="text-blue-200" /><h4 className="font-black uppercase tracking-tighter text-sm italic">TERAKTIF {monthName.toUpperCase()}</h4></div>
                  ...
                  <Link key={school._id} href={`/sekolah/${school.slug}`} className="flex items-center justify-between group">
                    <div className="flex items-center gap-4">
                      <div className="relative">
                        <div className="w-12 h-12 rounded-xl bg-white border border-slate-100 flex items-center justify-center p-1 group-hover:border-blue-500 transition-all overflow-hidden shadow-sm">
                      ...
                      <span className="text-[11px] font-black uppercase text-slate-800 line-clamp-1 group-hover:text-blue-600 transition-colors">{school.name}</span>
                    </div>
                    <div className="bg-white border border-blue-100 px-4 py-2 rounded-xl text-center min-w-[65px] group-hover:bg-blue-600 group-hover:text-white transition-all shadow-sm">
                      <span className="block text-lg font-black text-blue-600 group-hover:text-white leading-none">{school.mentionCount || 0}</span>
                      <span className="text-[7px] font-bold uppercase group-hover:text-blue-100 transition-colors">WARTA</span>
                    </div>
                  </Link>
                ))}
              </div>
            ...
            {/* WIDGET 3: TERPOPULER (HOT POSTS) - SUDAH HADIR! */}
            <div className="bg-white p-7 rounded-xl border border-slate-100 shadow-md">
              <div className="flex items-center gap-3 mb-6 border-b border-orange-50 pb-4">
                ...
                <h4 className="font-black text-slate-800 uppercase tracking-tighter text-sm italic">Postingan Terpopuler</h4>
              </div>
