...
const ptComponents = {
  block: {
    ...
    h2: ({ children }: any) => (
      <h2 className="text-2xl font-bold mt-10 mb-4 text-slate-800 uppercase border-l-4 border-blue-600 pl-4">{children}</h2>
    ),
    h3: ({ children }: any) => (
      <h3 className="text-xl font-bold mt-8 mb-3 text-slate-800">{children}</h3>
    ),
    blockquote: ({ children }: any) => (
      <blockquote className="border-l-4 border-blue-600 pl-4 italic my-8 text-gray-600 bg-slate-50 py-4 rounded-r-lg">
        {children}
      </blockquote>
    ),
  },
  ...
  if (!post) {
    return (
      <div className="container mx-auto py-40 text-center">
        <h1 className="text-3xl font-bold text-slate-300 uppercase tracking-widest mb-6">Artikel Tidak Ditemukan</h1>
    ...
    <article className="min-h-screen bg-white pb-24">
      <div className="max-w-4xl mx-auto px-4 md:px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {post.related && post.related.length > 0 ? (
              post.related.slice(0, 4).map((rel: any) => (
                <Link href={`/artikel/${rel.slug}`} key={rel._id} className="group flex flex-col h-full bg-white rounded-2xl overflow-hidden hover:shadow-xl transition-all duration-500 border border-slate-50">
              ...
              <p className="col-span-full text-slate-400 italic text-sm">Tidak ada artikel terkait lainnya.</p>
            )}
          </div>
        </div>
        ...
        <div className="mt-20 flex justify-center">
          <Link href="/artikel" className="group flex items-center gap-3 text-xs font-bold text-slate-500 uppercase tracking-[0.3em] hover:text-blue-600 transition-colors">
