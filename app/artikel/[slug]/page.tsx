...
// --- 3. KONFIGURASI PORTABLE TEXT (EDUCATION BLUE THEME) ---
const ptComponents = {
  types: {
    ...
    image: ({ value }: any) => {
      ...
      return (
        <div className="my-10 overflow-hidden rounded-2xl border-4 border-white shadow-xl ring-1 ring-blue-50 bg-slate-50">
  ...
  block: {
    ...
    blockquote: ({ children }: any) => (
      <blockquote className="border-l-4 border-blue-600 pl-6 italic my-10 text-blue-900 bg-blue-50/50 py-8 rounded-r-2xl shadow-inner font-medium text-xl">
        "{children}"
      </blockquote>
    ),
  },
...
export default async function ArtikelDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <article className="min-h-screen bg-white pb-24 font-sans pt-12 md:pt-20">
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        
        {/* HEADER SECTION (Bright & Airy) */}
        <header className="mb-12 max-w-4xl mx-auto md:mx-0">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 border-y border-slate-100 py-6">
            ...
            <div className="flex items-center gap-2 bg-blue-50 px-5 py-2.5 rounded-full border border-blue-100 shadow-inner">
              <Eye size={16} className="text-blue-600" />
              <span suppressHydrationWarning className="text-sm font-black text-blue-900">{post.views || 0} <span className="text-[10px] text-blue-400 ml-1 uppercase font-bold">Dilihat</span></span>
            </div>
          </div>
        </header>
            ...
            <div className="sticky top-28 space-y-8">
              ...
              {toc.length > 0 && (
                <div className="bg-slate-50 p-7 rounded-2xl border border-slate-100 shadow-sm">
                  <div className="flex items-center gap-3 mb-6 border-b border-blue-100 pb-4">
                    ...
                    <h4 className="font-black text-blue-900 uppercase tracking-tighter text-sm">Navigasi Artikel</h4>
                  </div>
              ...
              <div className="bg-white rounded-2xl shadow-2xl shadow-blue-900/10 border border-blue-100 overflow-hidden relative">
                    ...
                    <Link key={school._id} href={`/sekolah/${school.slug}`} className="flex items-center justify-between group">
                      <div className="flex items-center gap-4">
                        <div className="relative">
                          ...
                          <div className={`absolute -top-2 -right-2 w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-black text-white shadow-lg border-2 border-white transform group-hover:scale-110 transition-all ${idx === 0 ? 'bg-yellow-500' : 'bg-blue-600'}`}>{idx + 1}</div>
                        </div>
                ...
                <div className="p-4 bg-slate-50 border-t border-slate-100 text-center">
                  <p className="text-[9px] text-slate-400 font-bold uppercase tracking-[0.1em] italic">Statistik Berita Sekolah Bulan {monthName} 2026</p>
                </div>
              </div>
        ...
        <div className="mt-24 border-t border-slate-100 pt-16">
          <div className="flex items-center gap-3 mb-10">
            ...
            <h3 className="text-2xl font-black text-slate-800 uppercase tracking-tighter italic">Karya Guru Lainnya</h3>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {post.related?.map((rel: any) => (
              <Link href={`/artikel/${rel.slug}`} key={rel._id} className="group flex flex-col bg-white rounded-2xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-500">
        ...
        <div className="mt-24 flex justify-center pb-10">
          <Link href="/artikel" className="group flex items-center gap-6 text-[11px] font-black text-slate-400 uppercase tracking-[0.4em] hover:text-blue-600 transition-all">
            <div className="bg-slate-50 group-hover:bg-blue-600 group-hover:text-white p-5 rounded-full transition-all shadow-sm border border-slate-100">
              <ArrowLeft size={20} />
            </div>
            Daftar Artikel Guru
          </Link>
