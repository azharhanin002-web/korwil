  ...
  if (!post) {
    return (
      <div className="container mx-auto py-40 text-center">
        <h1 className="text-3xl font-bold text-slate-300 uppercase tracking-widest mb-6 italic">Kegiatan Tidak Ditemukan</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {post.related && post.related.length > 0 ? (
              post.related.slice(0, 4).map((rel: any) => (
                <Link href={`/pramuka/${rel.slug}`} key={rel._id} className="group flex flex-col h-full bg-white rounded-2xl overflow-hidden hover:shadow-xl transition-all duration-500 border border-slate-100">
              ...
              <p className="col-span-full text-slate-400 italic text-sm">Belum ada kegiatan terkait lainnya.</p>
            )}
          </div>
        </div>
