...
// --- 3. KONFIGURASI PORTABLE TEXT (FIX SPACING & SMART QUOTE PRAMUKA) ---
const ptComponents = {
  types: {
    ...
    image: ({ value }: any) => {
      ...
      return (
        <div className="my-12 overflow-hidden rounded-xl border-4 border-white shadow-xl ring-1 ring-orange-100 bg-slate-50">
          <Image 
          ...
          />
          {value.caption && <p className="bg-orange-50 py-4 text-center text-[10px] font-black uppercase tracking-widest text-orange-600 border-t border-orange-100">{value.caption}</p>}
        </div>
      );
    },
  },
  block: {
    h2: ({ children }: any) => {
      ...
      return <h2 id={id} className="text-2xl font-black mt-14 mb-6 text-[#5D4037] uppercase border-l-4 border-orange-500 pl-4 tracking-tight scroll-mt-28">{children}</h2>;
    },
    h3: ({ children }: any) => {
      const id = children[0].toString().toLowerCase().replace(/\s+/g, "-");
    ...
    normal: ({ children }: any) => {
      ...
      if (hasQuotes) {
        return (
          <p className="mb-7 leading-[1.8] text-[#3E2723] text-lg italic font-serif bg-orange-50/40 px-6 py-4 rounded-r-xl border-l-4 border-orange-300 shadow-sm last:mb-0">
            {children}
          </p>
        );
      }
    ...
    blockquote: ({ children }: any) => (
      <div className="relative my-12 group">
        <div className="absolute -left-2 top-0 bottom-0 w-1.5 bg-orange-600 rounded-full"></div>
        <blockquote className="pl-10 pr-6 py-10 bg-orange-50/50 rounded-r-2xl shadow-inner relative overflow-hidden">
          <Quote className="absolute -top-4 -left-2 w-20 h-20 text-orange-100/50 -rotate-12" />
          <p className="relative z-10 italic font-serif text-xl text-[#3E2723] leading-relaxed">{children}</p>
        </blockquote>
      </div>
    ),
  },
...
export default async function PramukaDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const currentUrl = `https://www.korwilbarat.web.id/pramuka/${slug}`;
  const isVideo = post.category === "Video";
  ...
  return (
    <main className="min-h-screen bg-white pb-24 font-sans pt-12 md:pt-20">
      <div className="max-w-7xl mx-auto px-4 md:px-6" suppressHydrationWarning>
        ...
        {/* --- 1. NAVIGASI (BREADCRUMB) - ANAK PERTAMA --- */}
        ...
        <header className="mb-16 max-w-4xl">
          <div className="flex justify-start mb-6">
            <div className="inline-flex items-center gap-2 bg-orange-100 text-orange-700 text-[10px] font-black px-4 py-2 rounded-lg uppercase tracking-[0.2em] shadow-lg border border-orange-200">
              <span className="animate-bounce text-xs">⚜️</span> Warta Pramuka
            </div>
          </div>

          <h1 className="text-3xl md:text-5xl font-black text-[#3E2723] leading-[1.15] mb-10 uppercase tracking-tighter italic">{post.title}</h1>
          ...
          <div className="flex flex-wrap items-center justify-between gap-6 border-y border-slate-100 py-8">
            ...
            <div className="flex items-center gap-3 bg-orange-50 px-6 py-3 rounded-full border border-orange-100 shadow-sm" suppressHydrationWarning>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
          <div className="lg:col-span-8 w-full">
            ...
            <div className="mt-20 pt-10 border-t border-slate-100 space-y-16">
               <div>
                  <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-8 italic">Bagikan Kegiatan:</h4>
                  <div className="flex justify-center md:justify-start"><ShareButtons url={currentUrl} title={post.title} /></div>
               </div>
               <div id="diskusikegiatan"><SupabaseComments postId={post.slug.current} /></div>
            </div>
          </div>
          ...
          <aside className="lg:col-span-4 w-full h-fit sticky top-28 space-y-12">
            {toc.length > 0 && (
              <div className="bg-slate-50 p-8 rounded-xl border border-slate-100 shadow-sm">
                <div className="flex items-center gap-3 mb-8 border-b border-orange-100 pb-4">
                  <List size={20} className="text-orange-600" /><h4 className="font-black text-[#5D4037] uppercase tracking-tighter text-sm italic">Isi Kegiatan</h4>
                </div>
                <nav className="space-y-4">
                  {toc.map((item: any, idx: number) => (
                    <Link key={idx} href={`#${item.id}`} className="group flex items-start gap-3 text-[13px] font-bold transition-all hover:text-orange-600 leading-tight text-slate-600">
            ...
            <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden relative">
              <div className="bg-gradient-to-r from-[#5D4037] to-[#3E2723] p-6 flex justify-between items-center text-white">
                <div className="flex items-center gap-3"><TrendingUp size={22} className="text-orange-300" /><h4 className="font-black uppercase tracking-tighter text-sm italic">TERAKTIF {monthName}</h4></div>
                  ...
                  <Link key={school._id} href={`/sekolah/${school.slug}`} className="flex items-center justify-between group">
                    <div className="flex items-center gap-4">
                      <div className="relative">
                        <div className="w-12 h-12 rounded-xl bg-white border border-slate-100 flex items-center justify-center p-1.5 group-hover:border-orange-500 transition-all overflow-hidden shadow-sm">
                        ...
                        <div className={`absolute -top-2 -right-2 w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-black text-white shadow-lg border-2 border-white ${idx === 0 ? 'bg-yellow-500' : 'bg-[#5D4037]'}`}>{idx + 1}</div>
                      </div>
                      <span className="text-[11px] font-black uppercase text-slate-800 line-clamp-1 group-hover:text-orange-600 transition-colors">{school.name}</span>
                    </div>
                    <div className="bg-white border border-slate-100 px-4 py-2 rounded-xl text-center min-w-[65px] group-hover:bg-orange-600 group-hover:text-white transition-all transform group-hover:scale-110 shadow-sm">
                      <span className="block text-lg font-black text-orange-600 group-hover:text-white leading-none">{school.mentionCount || 0}</span>
              ...
              <div className="p-5 bg-slate-50 border-t border-slate-100 flex flex-col items-center">
                 ...
                 <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest italic text-center leading-relaxed px-4">DATA UPDATE PERIODE {monthName} 2026</p>
              </div>
            </div>

            {/* WIDGET 3: HOT POSTS (TERPOPULER) - SUDAH KEMBALI! */}
            <div className="bg-white p-8 rounded-xl border border-slate-100 shadow-md">
              <div className="flex items-center gap-3 mb-8 border-b border-orange-50 pb-4">
                ...
                <h4 className="font-black text-slate-800 uppercase tracking-tighter text-sm italic">Paling Hot</h4>
              </div>
        ...
        <div className="mt-32 border-t border-slate-100 pt-20">
          <h3 className="text-2xl font-black text-[#3E2723] uppercase tracking-tighter mb-12 italic text-center md:text-left">Kegiatan Terkait</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {post.related?.map((rel: any) => (
              <Link href={`/pramuka/${rel.slug}`} key={rel._id} className="group flex flex-col bg-white rounded-xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-500 hover:-translate-y-2">
                ...
                <div className="p-6 flex flex-col flex-1">
                  <div className="text-[9px] font-black text-orange-600 uppercase mb-3 tracking-widest">{rel.category || "Pramuka"}</div>
        ...
        <div className="mt-24 flex justify-center pb-10">
          <Link href="/pramuka" className="group flex items-center gap-8 text-[11px] font-black text-slate-400 uppercase tracking-[0.4em] hover:text-orange-600 transition-all"><div className="bg-slate-50 group-hover:bg-[#5D4037] group-hover:text-white p-6 rounded-full shadow-sm border border-slate-100 transition-all active:scale-90"><ArrowLeft size={20} /></div>KEMBALI KE PRAMUKA</Link>
        </div>
      </div>
